...
The next step is to define the variables you will be working with from the previously uploaded data. There are different types of variables, the main three ones are: dependent (set_depVarName), date (set_dateVarName) and media variable names (media_vars).
...
Moreover, you will have to define which base variables (baseline_vars) typically competitors, price, promotion, temperature, unemployment rate, etc. you own to use. In the example simulated data we use competitor_sales_B as one of the baseline variables:

```
baseline_vars <- c("competitor_sales_B")
```
...
Finally, under ‘trials’ you will have to define the number of trials. 40 is recommended without calibration, 100 with calibration. If iterations <- 500 and trials <- 40, this means that we will have 40 different initialization trials, each of them with 500 iterations.
...
# calibration_lift <- data.table(channel = c("facebook_I",  "tv_S", "facebook_I"),
...
Once you have defined all the variables from previous steps, you will need to first prepare the input data within the ‘dt_mod’ object. Finally, you will have to execute the ‘robyn’ function in order to run the model.  Please set your preferred folder path (If the default "~/Documents/GitHub/plots" does not work out well for you) to save plots. Please notice the path has to end without a "/".
...
#### Run models
robyn_output <- robyn_run(hyper_bounds
                                ,optimizer = set_hyperOptimAlgo
                                ,trials = trials
                                ,set_cores = set_cores
