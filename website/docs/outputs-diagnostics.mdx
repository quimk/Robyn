...
The MMM code will automatically generate a set of plots under the folder you specify on the ‘robyn_output’ object. Each of these plots represents one of the optimal model solutions as a result of the multi-objective optimization Pareto optimal process mentioned in the **‘Automated hyperparameter selection and optimization’** section. Please find below an example of the model output:
